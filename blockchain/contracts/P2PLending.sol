...
contract P2PLending 
{
    ...
    function ifLoanOpen(uint index) public returns (uint) 
    {
        Loan memory loan = loans[index];
        if (loan.openLoan == true) return 1; else return 0;
    }
    ...
    function isUser(address account) public returns (uint) 
    {
	if(users[account].EXISTS == true) return 1; else return 0;
    }
